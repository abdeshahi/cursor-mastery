...
async function scrollNearFooter(page) {
  await page.evaluate(() => {
    const footer = document.querySelector('#footer');
    const y = footer ? footer.offsetTop - 40 : document.body.scrollHeight - 900;
...
const shots = [
  ...
  {
    name: '390-rail-mosaic-full',
    width: 390,
    prepare: (page) => scrollToSelector(page, '.cttel-home-rail'),
    height: 1180,
  },
  {
    name: '390-products-used',
    width: 390,
    prepare: (page) => scrollToSelector(page, '.cttel-home-products'),
    height: 920,
  },
  {
    name: '390-installment-trust-footer',
    width: 390,
    prepare: (page) => scrollToSelector(page, '.cttel-home-installment'),
    height: 980,
  },
  { name: '1440-header-hero', width: 1440, prepare: async (page) => page.evaluate(() => window.scrollTo(0, 0)) },
  {
    name: '1440-mosaic-products',
    width: 1440,
    prepare: (page) => scrollToSelector(page, '.cttel-home-mosaic'),
    height: 1020,
  },
  {
    name: '1440-lower-footer',
    width: 1440,
    prepare: (page) => scrollNearFooter(page),
    height: 980,
  },
];
