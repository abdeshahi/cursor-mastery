<?php
/**
 * Seed WooCommerce featured images from local cttel-ms product photos (staging helper).
 *
 * Only sets _thumbnail_id when the product has no featured image and no gallery.
 * Run inside wp_staging_app (never production):
 *
 *   docker exec wp_staging_app wp eval-file wp-content/mu-plugins/cttel-ms-seed-product-photos.php --allow-root
 *
 * (Canonical copy: wordpress/mu-plugins/cttel-ms-seed-product-photos.php)
 *
 * @package CTTEL
 */
...
if ( ! defined( 'ABSPATH' ) ) {
	...
	fwrite( STDERR, "Run via WP-CLI: wp eval-file seed-ms-product-photos.php\n" );
	exit( 1 );
}
